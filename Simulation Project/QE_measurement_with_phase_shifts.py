...
OUTPUT_DIR = r".\QE with phase shifts"
...
class LightDE:
    def __init__(self, loss, bounds):
        self.loss = loss
        self.bounds = bounds
        self.pop = np.array([[np.random.uniform(l, h) for l, h in bounds] for _ in range(EA_POP)])
...
def apply_phase_shift(S_list, delta_ratio):
    delta_phi = 2 * np.pi * delta_ratio
    S_shifted_list = []
    for S in S_list:
        fx, fy, phi0 = get_initial_fft(S)
        A = 8192.0
        B = 6000.0
        S_shifted = A + B * np.cos(phi0 + delta_phi + fx * X + fy * Y)
        S_shifted_list.append(S_shifted)
    return S_shifted_list

def get_initial_fft(S):
    data = S - np.mean(S)
    fft2 = np.fft.fft2(data)
    fft2 = np.fft.fftshift(fft2)
    amp = np.abs(fft2)
...
def test_with_phase_shift(shift_ratio, shift_tag, S1_list, I1_list, S2_list, I2_list, a_true, b_true, c_true):
    print(f"\n{'=' * 80}")
    print(f"相位偏移比例 = {shift_ratio:.2f}  →  Δφ = 2π × {shift_ratio:.2f}")
    print(f"{'=' * 80}")


    S1_shifted = apply_phase_shift(S1_list, shift_ratio)
    S2_shifted = apply_phase_shift(S2_list, shift_ratio)
    I1_clean = I1_list
    I2_clean = I2_list


    x0_1 = get_initial(I1_clean[0])
    x0_2 = get_initial(I2_clean[0])


    _, S1_ea = fit_evo(S1_shifted[0], x0_1)
    _, S2_ea = fit_evo(S2_shifted[0], x0_2)


    I1_crop = [crop_top_right(i) for i in I1_clean]
    I2_crop = [crop_top_right(i) for i in I2_clean]
    ...
    aE, bE, cE = fit_image(S1_ea_list, I1_crop, S2_ea_list, I2_crop, "ea")
    ...
    np.save(os.path.join(OUTPUT_DIR, f"a_{shift_tag}_ea.npy"), aE)
    np.save(os.path.join(OUTPUT_DIR, f"b_{shift_tag}_ea.npy"), bE)
    np.save(os.path.join(OUTPUT_DIR, f"c_{shift_tag}_ea.npy"), cE)
    # np.save(os.path.join(OUTPUT_DIR, f"QE_{shift_tag}_ea.npy"), QE)
...
if __name__ == "__main__":
    ...
    for ratio, tag in shift_list:
        test_with_phase_shift(ratio, tag, S1_list, I1_list, S2_list, I2_list, a_true, b_true, c_true)
