...
OUTPUT_DIR = r".\QE with light-intensity fluctuations"
...
def test_with_noise(eta, S1_list, I1_list, S2_list, I2_list, a_true, b_true, c_true):
    print(f"\n{'=' * 80}")
    print(f"高斯噪声强度 η = {eta} → 只加在 S 上，I 保持干净")
    print(f"{'=' * 80}")


    S1_noisy_list = [add_gaussian_noise(s, eta) for s in S1_list]
    S2_noisy_list = [add_gaussian_noise(s, eta) for s in S2_list]


    I1_clean_list = I1_list
    I2_clean_list = I2_list


    x0_1 = get_initial(I1_clean_list[0])
    x0_2 = get_initial(I2_clean_list[0])


    _, S1_ea = fit_evo(S1_noisy_list[0], x0_1)
    _, S2_ea = fit_evo(S2_noisy_list[0], x0_2)


    I1_crop_list = [crop_top_right(i) for i in I1_clean_list]
    I2_crop_list = [crop_top_right(i) for i in I2_clean_list]
